...
def purchaseOneItem(howmany: int = 1):
...
class ShoppingCart:    
    def __init__(self):
        ...
        self.cart_items: List[ItemToPurchase] = [] # as stated in the problem, it should list. So, here I'll use above 'CartItem' object as each item in the list.
    ...
    def modify_item(self, modifyingItem: ItemToPurchase ):
        if modifyingItem != None and len(self.cart_items) > 0:
            ...
            if(indx >= 0):
                self.cart_items[indx].Description = modifyingItem.Description
                self.cart_items[indx].item_price = modifyingItem.item_price
                self.cart_items[indx].item_quantity = modifyingItem.item_quantity            
            else:
                print("Item not found in cart")
        return
    ...
    def print_total(self):
        ...
        totalItems = 0
        for anItem in self.cart_items:
            totalItems += anItem.item_quantity
...
def module8pp_OnlineShoppingCart():
    clearScreen()
    print("\n === Shopping Cart Demo ===")
    ShowStoreItems()
